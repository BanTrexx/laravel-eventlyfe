...
@section('title', 'Daftar sebagai Kreator - EventLyfe')
            ...
            <div class="card border-0 shadow-lg rounded-4 bg-surface">
                <div class="card-body p-4 p-md-5">
                    ...
                    <div class="text-center mb-4">
                        ...
                        <h3 class="fw-bold text-auth-header">Mulai Buat Eventmu!</h3>
                        <p class="text-muted small">Daftar sebagai Organizer dan jangkau ribuan penonton.</p>
                    </div>
                    ...
                    <form method="POST" action="{{ route('register') }}">
                        ...
                        <div class="mb-3">
                            <label for="full_name" class="form-label fw-bold small text-auth-label">Nama Organisasi / Nama Lengkap</label>
                            <div class="input-group">
                                <span class="input-group-text bg-input-custom border-0"><i class="bi bi-building text-muted"></i></span>
                                <input id="full_name" type="text" class="form-control bg-input-custom border-0 @error('full_name') is-invalid @enderror"
                                    name="full_name" value="{{ old('full_name') }}" required autocomplete="name" autofocus placeholder="Contoh: Java Jazz Production">
                            </div>
                            @error('full_name')
                        ...
                        <div class="mb-3">
                            ...
                            <div class="input-group">
                                ...
                                <input id="username" type="text" class="form-control bg-input-custom border-0 @error('username') is-invalid @enderror"
                                    name="username" value="{{ old('username') }}" required placeholder="username_kreator">
                            </div>
                            @error('username')
                            <span class="invalid-feedback d-block"><strong>{{ $message }}</strong></span>
                            @enderror
                        ...
                        <div class="mb-3">
                            <label for="email" class="form-label fw-bold small text-auth-label">Alamat Email Bisnis</label>
                            <div class="input-group">
                                ...
                                <input id="email" type="email" class="form-control bg-input-custom border-0 @error('email') is-invalid @enderror"
                                    name="email" value="{{ old('email') }}" required placeholder="organizer@email.com">
                            </div>
                            @error('email')
                            <span class="invalid-feedback d-block"><strong>{{ $message }}</strong></span>
                            @enderror
